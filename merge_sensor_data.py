...
def merge_sensor_data(dir_in, dir_out):
    ...
    for sub_dir_ in sub_dir:
        ...
        file_summary = pd.DataFrame({'file_path': file_path,
                                     'file_name': file_name,
        ...
        for key, val in data.items():
            ...
            data[key].columns = [x + '_' + key if 'datetime' not in x else x for x in data[key].columns]

        data_merged = reduce(lambda left, right: pd.merge(left,
                                                          right,
